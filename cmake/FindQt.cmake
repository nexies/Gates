...
function(_qt_find_platform_exclude_matches _manifest _exclude _out)
    if(NOT _exclude)
        set(${_out} FALSE PARENT_SCOPE)
        return()
    endif()
    ...
    # _exclude may arrive as a CMake list (semicolons) or space-separated string
    string(REPLACE ";" " " _exclude_flat "${_exclude}")
    _qt_find_normalize_tokens("${_exclude_flat}" _exclude_norm)

    if(_exclude_norm STREQUAL "")
        set(${_out} FALSE PARENT_SCOPE)
        return()
    endif()

    string(REPLACE " " ";" _tokens "${_exclude_norm}")
...
function(_qt_find_discover _out_ids _roots _verbose _nocache)
    ...
    foreach(_qmake IN LISTS _qmake_candidates)
        ...
        if(_manifest MATCHES "llvm")
            string(APPEND _manifest " llvm")
        endif()
...
function(qt_find)
    ...
    set(_multi COMPONENTS PLATFORM_EXCLUDE)
    ...
    _qt_find_log("${_verbose}" "STATUS" "Requested exclude   : ${QTF_PLATFORM_EXCLUDE}")
    ...
    foreach(_rid IN LISTS _ids)
        ...
        if(QTF_PLATFORM_EXCLUDE)
            _qt_find_platform_exclude_matches("${_manifest}" "${QTF_PLATFORM_EXCLUDE}" _should_exclude)
            if(_should_exclude)
                _qt_find_log("${_verbose}" "STATUS" "Reject ${_kit_id}: manifest matches PLATFORM_EXCLUDE '${QTF_PLATFORM_EXCLUDE}'")
                continue()
            endif()
        endif()
